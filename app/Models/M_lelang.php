		...
		public function join($table, $table2, $on){
			return $this->db->table($table)
                            ...
                            public function joinWhere($table, $table2, $on, $where) {
                                ...
                                ->join($table2, $on, 'left')
                                ->getWhere($where)
                                ->getResult();  // This returns all the rows
...
public function hitungSemuaPermainan() 
{
    return $this->db->table('permainan')->where('deleted_at', null)->countAllResults();
}

public function hitungSemuaPelanggan() 
{
    return $this->db->table('pelanggan  ')->where('deleted_at', null)->countAllResults();
}
...
public function hitungTransaksiHariIni()
{
    ...
    return $this->db->table('transaksi') // Mengakses tabel transaksi
        ->where('deleted_at', null) // Hanya menghitung data yang tidak dihapus secara soft-delete
        ->where('created_at >=', $hariIni) // Tanggal mulai dari hari ini
        ->where('created_at <', $besok) // Tanggal sebelum besok
        ->countAllResults();
}
